...
from Task_Tester import run_task_tester
# -----------------------------
# Load resources
# -----------------------------
...
def render_node(resource):
    ...
    if not children_lists:
        return html.Li([html.Span(resource.name, style={"fontWeight": "bold"}), render_state_kv(state)])
    return html.Li([
        html.Span(resource.name, style={"fontWeight": "bold"}),
        render_state_kv(state),
        html.Ul([render_node(child) for child_list in children_lists for child in child_list],
...
app.layout = html.Div([
    ...
    html.Button("Run Experiment", id="run-exp-btn", n_clicks=0),
    html.Div(id="exp-status", style={"marginTop": "10px"}),
    dcc.Interval(id="refresh", interval=500, n_intervals=0),
    html.Div(id="tree-view", style={"fontFamily": "monospace", "color": "black"})
])
...
def update_tree(_):
    roots = find_roots()
    return html.Ul([render_node(r) for r in roots], style={"listStyleType": "none", "paddingLeft": "0px"})
...
def start_experiment(n_clicks):
    if n_clicks > 0 and not experiment_running:
