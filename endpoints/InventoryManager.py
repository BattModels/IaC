...
@register_resource("inventory_manager")
class InventoryManager(Resource):
    ...
    def update(self, mode, *args, **kwargs):
        """
        ...
        if self.inventory is None:
            ...
            self.connect()
        try:
            if mode == InventoryManager.InventoryMode.FIND_COMPONENT:
            ...
            elif mode == InventoryManager.InventoryMode.EDIT_INVENTORY:
            ...
            elif mode == InventoryManager.InventoryMode.SOLVE_LIST:
    ...
    def read(self):
        if self.inventory is None:
            ...
            self.connect()
        self.inventory = pd.read_csv(self.inventory_path)
        allowed_fields = ["CompositionID", "Density (g/mL)", "Volume (mL)"]
